...
function fs.complete(sPath, sLocation, bIncludeFiles, bIncludeDirs)
    ...
    if fs.isDir(sDir) then
        ...
        local nNameLen = text_len(sName)
        for n = 1, #tFiles do
            local sFile = tFiles[n]
            local nFileLen = text_len(sFile)
            if nFileLen >= nNameLen and text_sub(sFile, 1, nNameLen) == sName and (
                bIncludeHidden or text_sub(sFile, 1, 1) ~= "." or text_sub(sName, 1, 1) == "."
            ) then
                local bIsDir = fs.isDir(fs.combine(sDir, sFile))
                local sResult = text_sub(sFile, nNameLen + 1)
                if bIsDir then
                    table.insert(tResults, sResult .. "/")
                    if bIncludeDirs and text_len(sResult) > 0 then
                        table.insert(tResults, sResult)
                    end
                else
                    if bIncludeFiles and text_len(sResult) > 0 then
                        table.insert(tResults, sResult)
                    end
                end
            end
