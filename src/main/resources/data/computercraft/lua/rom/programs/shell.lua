...
local function completeProgram(sLine)
    ...
    if #sLine > 0 and (sLine:find("/") or sLine:find("\\")) then
        ...
        local nLineLen = text_len(sLine)

        -- Add aliases
        for sAlias in pairs(tAliases) do
            if text_len(sAlias) > nLineLen and text_sub(sAlias, 1, nLineLen) == sLine then
                local sResult = text_sub(sAlias, nLineLen + 1)
        ...
        for n = 1, #tPrograms do
            ...
            if text_len(sProgram) > nLineLen and text_sub(sProgram, 1, nLineLen) == sLine then
                local sResult = text_sub(sProgram, nLineLen + 1)
                if not tSeen[sResult] then
                    table.insert(tResults, sResult)
                    tSeen[sResult] = true
                end
