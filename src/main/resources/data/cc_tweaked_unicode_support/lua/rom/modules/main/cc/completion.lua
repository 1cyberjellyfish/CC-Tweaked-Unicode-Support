...
local function choice_impl(text, choices, add_space)
    local results = {}
    local nTextLen = text_len(text)
    for n = 1, #choices do
        local option = choices[n]
        local nOptLen = text_len(option)
        if nOptLen + (add_space and 1 or 0) > nTextLen and text_sub(option, 1, nTextLen) == text then
            local result = text_sub(option, nTextLen + 1)
            if add_space then
