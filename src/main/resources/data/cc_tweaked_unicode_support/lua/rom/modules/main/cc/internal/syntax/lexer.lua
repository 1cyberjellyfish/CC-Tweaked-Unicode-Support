...
local byte_len = unicode and unicode.byte_len or string.len
...
lex_string_zap = function(context, str, pos, start_pos, quote)
    while true do
        ...
        if not next_pos then
            context.report(errors.unfinished_string, start_pos, byte_len(str), quote)
            return tokens.STRING, byte_len(str), nil, { lex_string_zap, 1, 1, quote }
        end
...
local function lex_long_string(context, str, pos, start_pos, boundary_length)
    ...
    return tokens.STRING, byte_len(str), nil, { lex_long_string, 0, 0, boundary_length }
end
...
local function lex_long_comment(context, str, pos, start_pos, boundary_length)
    ...
    return tokens.COMMENT, byte_len(str), nil, { lex_long_comment, 0, 0, boundary_length }
end
...
local function lex_token(context, str, pos)
    ...
    elseif c == "+" then return tokens.ADD, pos
        ...
        if end_pos then end_pos = end_pos - 1 else end_pos = byte_len(str) end
