...
local function redrawLines(line, endLine)
    ...
    while (changed or line <= endLine) and line - scrollY < h do
        ...
        while token do
            ...
            if not finish or finish < pos then
                pos = pos + 1
            else
                pos = finish + 1
            end
            ...
            if pos > (unicode and unicode.byte_len(contents) or #contents) then break end
...
while bRunning do
    ...
    elseif event[1] == "char" then
        ...
        elseif not bReadOnly then
            ...
            setCursor(x + text_len(event[2]), y)
        end
