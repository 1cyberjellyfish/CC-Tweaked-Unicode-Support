...
function create(parent, nX, nY, nWidth, nHeight, bStartVisible)
    ...
    local function redrawLine(n)
        ...
        local sLineText = tLine[1]
        local sLineTextColor = tLine[2]
        local sLineBackgroundColor = tLine[3]

        local nLen1 = text_len(sLineText)
        local nLen2 = text_len(sLineTextColor)
        local nLen3 = text_len(sLineBackgroundColor)

        if nLen1 ~= nWidth or nLen2 ~= nWidth or nLen3 ~= nWidth then
            if nLen1 < nWidth then
                sLineText = sLineText .. string_rep(" ", nWidth - nLen1)
            elseif nLen1 > nWidth then
                sLineText = text_sub(sLineText, 1, nWidth)
            end

            if nLen2 < nWidth then
                sLineTextColor = sLineTextColor .. string_rep(tHex[nTextColor] or "0", nWidth - nLen2)
            elseif nLen2 > nWidth then
                sLineTextColor = string_sub(sLineTextColor, 1, nWidth)
            end

            if nLen3 < nWidth then
                sLineBackgroundColor = sLineBackgroundColor .. string_rep(tHex[nBackgroundColor] or "f", nWidth - nLen3)
            elseif nLen3 > nWidth then
                sLineBackgroundColor = string_sub(sLineBackgroundColor, 1, nWidth)
            end

            tLine[1] = sLineText
            tLine[2] = sLineTextColor
            tLine[3] = sLineBackgroundColor
        end

        parent.setCursorPos(nX, nY + n - 1)
        parent.blit(sLineText, sLineTextColor, sLineBackgroundColor)
    end
    ...
    function window.blit(sText, sTextColor, sBackgroundColor)
        ...
        local nTextLen = text_len(sText)
        local nColorLen1 = text_len(sTextColor)
        local nColorLen2 = text_len(sBackgroundColor)

        if nColorLen1 ~= nTextLen or nColorLen2 ~= nTextLen then
            if nColorLen1 < nTextLen then
                local lastChar = nColorLen1 > 0 and string_sub(sTextColor, -1) or (tHex[nTextColor] or "0")
                sTextColor = sTextColor .. string_rep(lastChar, nTextLen - nColorLen1)
            elseif nColorLen1 > nTextLen then
                sTextColor = string_sub(sTextColor, 1, nTextLen)
            end

            if nColorLen2 < nTextLen then
                local lastChar = nColorLen2 > 0 and string_sub(sBackgroundColor, -1) or (tHex[nBackgroundColor] or "f")
                sBackgroundColor = sBackgroundColor .. string_rep(lastChar, nTextLen - nColorLen2)
            elseif nColorLen2 > nTextLen then
                sBackgroundColor = string_sub(sBackgroundColor, 1, nTextLen)
            end
        end
