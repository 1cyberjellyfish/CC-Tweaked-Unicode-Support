...
function slowWrite(text, rate)
    ...
    for n = 1, text_len(wrapped_str) do
        sleep(to_sleep)
        write(text_sub(wrapped_str, n, n))
    end
end
    ...
    for n, t in ipairs(tAll) do
        if type(t) == "table" then
            for nu, sItem in pairs(t) do
                ...
                nMaxLen = math.max(text_len(tostring(sItem)) + 1, nMaxLen)
            end
        end
    end
...
function urlEncode(str)
    expect(1, str, "string")
    local gsub, byte, format = string.gsub, string.byte, string.format

    str = gsub(str, "\n", "\r\n")
    str = gsub(str, "[^A-Za-z0-9%-%_%.]", function(c)
        if c == " " then return "+" end

        -- Strings already contain UTF-8, so encode the current byte exactly
        -- once. Re-encoding bytes above 0x7f would corrupt Unicode URLs.
...
function complete(sSearchText, tSearchTable)
    ...
    local nPartLength = text_len(sPart)
    ...
    while tTable do
        for k, v in pairs(tTable) do
            if not tSeen[k] and type(k) == "string" then
                if text_len(k) >= nPartLength and text_sub(k, 1, nPartLength) == sPart then
                    if not g_tLuaKeywords[k] and string.find(k, "^[^%s%c%[%]%(%){}%.,:]+$") then
                        local sResult = text_sub(k, nPartLength + 1)
                        if nColon then
